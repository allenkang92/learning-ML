...
## 7. 소프트맥스 회귀 (다중 클래스 분류 단층 퍼셉트론)를 이용한 AND 게이트 구현

소프트맥스 회귀는 로지스틱 회귀를 다중 클래스 분류 문제로 확장한 것으로, 단층 퍼셉트론 구조를 사용합니다.

### 7.1 모델 구성 요소

1. **입력 및 출력:**
   - **입력 (X):** AND 게이트 입력값 `[[0,0], [0,1], [1,0], [1,1]]`
   - **정답 (Y):** AND 연산 결과를 원-핫 인코딩으로 표현 `[[1,0], [1,0], [1,0], [0,1]]`
     - `[1,0]`은 클래스 0 (False), `[0,1]`은 클래스 1 (True)

2. **가중치와 편향:**
   - **가중치 (W):** `(2, 2)` 형태 행렬, 각 입력 특성이 각 출력 클래스에 연결되는 가중치
   - **편향 (B):** `(2,)` 형태 벡터, 각 출력 클래스 노드에 더해지는 편향
   ```python
...
3. **활성화 함수:**
   - **소프트맥스 함수:** 선형합의 결과를 각 클래스에 대한 확률 분포로 변환
   ```python
   def softmax(x):
       if x.ndim == 1:
...
4. **손실 함수:**
   - **교차 엔트로피 (Cross-Entropy):** 다중 클래스 분류에 적합한 손실 함수
   ```python
   CE = lambda truey, predy: -np.sum(truey * np.log(predy + 1e-7))
   ```
...
1. **순전파:**
   - 가중합 계산: `ls = X @ W + B`
   - 확률 계산: `predY = softmax(ls)`

2. **역전파:**
   - 소프트맥스+교차엔트로피 기울기: `dY = predY - Y` (예측값 - 실제값)
   - 가중치 기울기: `dW = X.T @ dY`
   - 편향 기울기: `dB = np.mean(dY, axis=0)`
   - 가중치/편향 업데이트: `W -= lr * dW`, `B -= lr * dB`

3. **수치 안정성:**
   - 소프트맥스 함수는 큰 입력값에 대해 오버플로우 위험이 있어 **max trick** 적용
   - 교차 엔트로피 계산 시 작은 값 추가하여 로그 함수의 발산 방지

### 7.3 모델 평가

1. 최종 예측 확률: `final_pred = softmax(X @ W + B)`
2. 예측 클래스: `pred_class = np.argmax(final_pred, axis=1)`
3. 결정 경계 시각화: 입력 공간의 각 지점에서 클래스 1의 확률을 계산하여 등고선 그리기

### 7.4 소프트맥스 vs. 로지스틱 회귀

- **확장성:**
  - 로지스틱 회귀: 이진 분류(2개 클래스)만 가능
  - 소프트맥스 회귀: 다중 클래스(2개 이상) 분류 가능

- **출력 형태:**
  - 로지스틱 회귀: 단일 확률값 출력 (특정 클래스에 속할 확률)
  - 소프트맥스 회귀: 클래스 수만큼의 확률값 출력 (모든 클래스에 대한 확률 분포)

- **손실 함수:**
  - 로지스틱 회귀: 이진 크로스 엔트로피 (BCE)
  - 소프트맥스 회귀: 다중 클래스 크로스 엔트로피 (CE)

- **실제 구현은 `softmax_and_gate.py` 파일에서 확인 가능**
